...
7. 크로스-플랫폼 구조화된 데이터 표현(json/avro/protobuf)을 사용합시다.
...
## 06. 그러나 역직렬화는 가능하면, 사용하지 말아야 합니다.
...
대신에, 객체와 바이트 시퀀스를 변환해주는 다른 매커니즘 사용하면 됩니다. 이런 메커니즘을 직렬화 시스템이라고 불리기도 하고, 이펙티브 자바에서는 자바 직렬화와 구분하기 위해 크**로스-플랫폼 구조화된 데이터 표현**라고 표현합니다.
...
객체를 생성하는 건 `new` 뿐이라고 생각이 되는가? 아니다. **자바 직렬화를 활용하면 객체 생성자 역할을 할 수 있다!!** 
